...
export interface TermLine {
  kind: LineKind;
  text: string;
  dim?: boolean; // muted colour for file headers / secondary output
  // neofetch only
  logoChunk?: string;
  infoChunk?: string;
  infoStyle?: string;
...
export class HeroComponent implements OnDestroy {
  ...
  // Ghost text shown after the cursor — history match first, then known commands.
  // history is a plain array so this only reacts to inputValue changes, which is correct:
  // history updates happen on submit (which clears inputValue), so the next read is fresh.
  ...
  constructor() {
    effect(() => {
      this.lang.lang(); // reactive: restart animation on language switch
      this.reset();
    });
  }
  ...
  private tick(): void {
    ...
    if (entry.isPrompt) {
      if (this.charIdx <= entry.text.length) {
        ...
        // inject neofetch block immediately after neofetch command
        if (entry.isNeofetch) {
          this.lines.update((l) => [
            ...l,
